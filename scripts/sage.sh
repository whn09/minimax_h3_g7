...
# TP=4 x ULYSSES=2, 768p, 121 f (5.04 s), seed 42, 25 steps, reference short edge at the 2048
# default. The comparison is therefore against the recorded arms and nothing was re-run to make it:
...
# the customer can actually listen to.
#
...
# ARMS="S2" re-runs one arm without paying for the other. Added when the t2va prompt was swapped to
# @wide: S1 was already measured and re-serving it costs 90 s of startup plus 134 s of render to
# reproduce a number that is already in G7.md.
want() { case " ${ARMS:-S1 S2} " in *" $1 "*) return 0 ;; *) return 1 ;; esac; }
...
if want S2; then
echo "=== S2  t2va@wide + sage  (vs ir25 = 105.4 s; prompt swapped, see header)" | tee -a $R
...
sleep 15
if up $log && assert_sage $log; then
  # label=wide, because case_t2va_v2.txt holds several t2va arms and task= alone renders them all.
  python $V/sglang_case.py case=$V/case_t2va_v2.txt task=t2va label=wide tag=sage 768:25:121 2>&1 | tee -a $R
fi
stop
fi
